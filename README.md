...
## Veritas InfoScale (Arctera)

📚 **[Complete Documentation → docs/veritas-infoscale-guide.md](docs/veritas-infoscale-guide.md)**  
📊 **[OCPNAS-312 Validation Summary → docs/OCPNAS-312-VALIDATION-SUMMARY.md](docs/OCPNAS-312-VALIDATION-SUMMARY.md)**
...
| `make veritas TAGS=install` | Install InfoScale operator + cluster (auto-detect disks) | InfoScale stack only |
| `make veritas TAGS=install EXTRA_VARS="-e manual_disk_selection=true"` | Install with interactive disk selection | Choose specific disks on install |
| `make veritas TAGS=cluster-recreate` | Delete & recreate cluster (auto-detect disks) | Change disk configuration |
| `make veritas TAGS=cluster-recreate  EXTRA_VARS="-e manual_disk_selection=true"` | Interactive disk selection | Choose specific disks |
...
```bash
# Use existing OCP cluster (auto-detect disks)
make veritas TAGS=dependencies,install

# Use existing OCP cluster (manual disk selection)
...
# Override StorageClass parameters (optional)
infoscale_layout: "stripe"              # stripe, mirror, stripe-mirror, etc.
infoscale_nstripe: 2                    # Number of stripe columns
infoscale_fault_tolerance: 1            # Fault tolerance level
infoscale_media_type: "SSD"             # HDD or SSD
```

**Auto-Configuration:**
- **2+ disks**: Automatically sets `layout: stripe` and `nstripe: <disk_count>`
- **1 disk**: Uses InfoScale defaults (no layout/nstripe specified)
...
make veritas TAGS=install EXTRA_VARS="-e infoscale_layout=mirror"
```

**Available Layouts:**
- `stripe`: RAID 0 (performance, no redundancy)
- `mirror`: RAID 1 (redundancy)
- `stripe-mirror`: RAID 10 (performance + redundancy)
- `mirror-stripe`: RAID 01
- `concat`: Concatenation
- `concat-mirror`: Concatenated mirrors
- `mirror-concat`: Mirrored concatenation
